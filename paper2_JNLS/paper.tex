...
\begin{align}
\partial_t \Fcnl = \{ \Fcnl, \Ham \} = \int_{-\pi}^{\pi} \vard{F}{u} \sympJ \vard{\Ham}{u} \dx
\end{align}
where again $\Ham = \Hup$ if $t<0$ and $\Ham = \Hdn$ if $t>0$.
%----------------------------------------------------------------------------%
% Truncation
%----------------------------------------------------------------------------%
...
\end{align}
where the new symplectic operator is $\SympL = \partial_x \Proj$.
...
The KdV system (i.e. $\Lambda = \infty$) possesses the Hamiltonian \eqref{Hamiltonian} and momentum and energy \eqref{MomEn} and an infinite number of additional invariants \cite{lax1975periodic, whitham2011linear}. For the truncated system however, $\Lambda < \infty$, the only known invariants are the momentum, energy, and Hamiltonian. The infinite sequence of invariants apart from these first three may no longer be invariants of TKdV.


WRITE PROJECTION OPERATOR AND TKDV
