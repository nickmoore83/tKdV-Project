...
Thus, any of the choices $\lamfac = \Lambda$, $\Lambda/2$, or $\Lambda/4$, discussed in that section would be valid. In particular, if $\lamfac = \Lambda$, then the characteristic wavelength in experiments corresponds to the smallest resolved wavelength in the dynamical system. It is perhaps more sensible to choose $\lamfac$ to be an intermediate value between 1 and $\Lambda$, so that some scales both larger than and smaller than the characteristic wavelength $\lam$ are resolved. As default, we will choose $\lamfac = \Lambda/2$, so that on a log-scale, the characteristic wavelength lies directly in the middle of the resolved wavelengths. 
	...
	We note that the experiments discussed here preceded the development of the theory, and hence had no intent of mimicking a Gibbs measure, such as that in \eqref{Gibbs}. We expect that if the driving featured in the experiments were designed to mimic the Gibbs measure, in particular the spectral decay, then it would be more straightforward to assign a value to the model parameter $\lamfac$. We, however, leave that task for future research due to the significant cost of performing an entirely new set of experiments compared to the relative ease and great value in re-analyzing existing experimental measurements in light of the new theoretical developments.
...
\subsection{Statistical comparison between theory and experiments}
	...
	We now aim to compare the statistics measured in the experiments versus the theory. We focus on the representative set of experiments, detailed in Section \ref{experiments} and Table \ref{paramtable}. In these experiments, the peak downstream skewness is precisely 0.83. According to Fig.~\ref{transfig}, we can achieve that value of skewness for the case $(\Lambda, \lamfac) = (16, 8)$ by setting $\thup = 17$, which gives $\thdn = 13.5$. We thus run the DNS with these model parameters (and with a sufficiently fine timestep, and number of samples, and to a final dimensionless time of 10).


\vsp{5}

Figure \ref{uhist} shows histograms of the displacement $u$ from experiments and from TKdV theory for comparison. The experiment histograms representative exactly the same data as shown in Fig.~\ref{ExpDiagStats}, only converted to the dimensionless variables. As mentioned previously, the upstream histogram is nearly symmetric, while downstream histogram skews strongly towards positive displacement. The TKdV histograms do a remarkable job of recovering these features, appearing to produce nearly exactly the same distributions, both in shape and in scale.
%
