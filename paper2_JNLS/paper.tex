...
\subsection{Matching at the ADC}
	...
	Recall that the abrupt depth change is met by traveling waves at time $T_{ADC} = 0$, set to zero for convenience. The KdV equations describe wave dynamics over {\em long} timescales, $t \gg \timescale$, capturing weakly nonlinear and weakly dispersive effects. Meanwhile, evolution over shorter timescales is simply described by linear theory. The event of a wave crossing the ADC is precisely such a short-time event, and so we will employ matching conditions at the ADC that result from linear theory. 
...
We call \eqref{detmatch} the deterministic matching condition to contrast with its statistical counterpart introduced below. This matching condition is employed in the deterministic simulations of the TKdV dynamical system \eqref{TKdV}. We note that in the alternate formulation mentioned earlier, it is the product $\depth^{1/4} \eta$ that would match at the ADC \cite{johnson1997modern}.
	...
	Now, from the perspective of statistical mechanics, consider the communication between the statistical ensembles, $\Gupdn$, upstream and downstream of the ADC. These two systems are in contact at the ADC, and so the upstream state $\Gup$ can be regarded as a thermal reservoir that influences the downstream state $\Gdn$. In fact, the above deterministic  condition directly leads to a simple description for the link. The quantity of interest is the outgoing Hamiltonian $\Hdn$, since its value at $t=0^{-}$ is set by the upstream dynamics, and then this particular value is conserved thereafter in the downstream dynamics. Since $u$ matches at the ADC, so must $\Hdn$, and, in fact, this matching holds for every individual trajectory. Recall that $\Hdn(t)$ is not conserved in the upstream dynamics and so its value varies for $t < 0$. However, appealing to weak Ergodicity of the system, the ensemble mean $\meanup{\Hdn(t)}$ is expected to be independent of time. In particular, the value $\meanup{\Hdn \vert_{t=0^{-}} }$ is the same as the bare ensemble mean $\meanup{\Hdn}$. Afterwards, the particular value $\Hdn \vert_{t=0^{-}}$ is conserved in the downstream dynamics, on a trajectory-by-trajectory basis and for all time. Thus, the downstream measure $\Gdn$ must recover the ensemble mean $\meanup{\Hdn \vert_{t=0^{-}} }$, which gives the statistical matching condition
\begin{align}
\label{statmatch}
...
\end{equation}
Both the experiments and simulations show the upstream skewness to be negligible compared to the downstream skewness, allowing the term with $\meanup{\Hthree }$ to be dropped in \eqref{statmatchexp}. With this approximation, \eqref{statmatchexp} yields the relationship
\begin{equation}
\label{H3H2ratio}
...
\end{align}
This equation simply says that the expected value of $\Hthree$ is directly related to the outgoing skewness.
...
\end{equation}
This formula links the skewness of the outgoing wave field to the change in the variance of surface slope. In particular, it predicts that their ratio to scale as the {\em inverse cube of wave amplitude} and the {\em cube of the depth ratio}, as seen on the right-hand-side of the equation. It is a rather unexpected relationship, as it would be difficult to anticipate that the displacement skewness and slope variance, among all possibilities, are so intimately related.
