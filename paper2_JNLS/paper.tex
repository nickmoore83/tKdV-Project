...
% Data from column 5 in MasterTimeSeries, Delta theta = 1.38 degrees, which is in between runs 5 and 6.
	...
	The free surface is illuminated by light-emitting diodes that run along the bottom of the tank and is then imaged from the sideview with a Nikon D3300 at 60 frames per second. The illumination technique, coupled with high pixel count of the camera, allows surface displacements to be resolved with accuracy better than 1/3 millimeter. Furthermore, these optical measurements permit extraction of wave statistics {\em continuously} in space, rather than at a few discrete locations. Further details of the experimental setup can be found in \cite{bolles2019}.

	Example measurements of free-surface displacements $\eta$ are shown in Figs.~\ref{ExpDiagStats}(b)--(c). These measurements are taken at two representative locations: one a short distance upstream of the ADC and one downstream of the ADC. Both signals exhibit a combination of periodic and random fluctuations, with the dominant oscillations corresponding to the peak driving frequency of 2 Hz. The nature of the random fluctuations is revealed by the corresponding histograms shown in Fig.~\ref{ExpDiagStats}(d)--(e). The upstream measurements are symmetrically distributed about the mean displacement of $\eta = 0$. In fact, Bolles et al.~found that these measurements closely follow a Gaussian distribution \cite{bolles2019}. The downstream measurements, however, are not symmetrically distributed and skew strongly towards positive displacement, $\eta > 0$. Bolles et al.~found that these measurements can be well approximated by a mean-zero gamma distribution \cite{bolles2019}.
