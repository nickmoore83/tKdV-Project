...
\begin{abstract} 
Recent laboratory experiments of \boetal revealed the emergence of anomalous statistics in a randomized wave-field that encounters an abrupt change in bottom topography. \maetal established a theoretical foundation, based on analysis of the truncated Korteweg–de Vries (TKdV) system, that successfully captures several key qualitative features of the experiments, including the robust emergence of anomalous statistics in the outgoing waves. The framework exploits the Hamiltonian structure of TKdV to construct invariant measures that are statistical matched at the depth change, and is corroborated by numerical simulations of the deterministic dynamics. 
Here we present a more comprehensive treatment of these parallel experimental and modeling efforts, with emphasis on the link between the two. We present an assortment of new experimental measurements and carefully compare them to model predictions. We establish a precise link between experimental and model parameters, so that the invariant measure underlying the model can be calibrated to the specific conditions present in experiments and a quantitative comparison can be made. The model predicts a peculiar relationship between surface-displacement skewness and surface-slope variance, which is confirmed in a spectacular fashion by new experimental measurements.
\end{abstract}
