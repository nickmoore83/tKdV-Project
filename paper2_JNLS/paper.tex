...
\newcommand{\usqdah}{ (\widehat{u_{da}^2}) }
...
\begin{align}
\label{TKdV2}
&\pd{\uL}{t} = -  \frac{1}{2} C_3 \drat^{-3/2} \, \pd{}{x} \Proj (\uL)^2 - C_2 \drat^{1/2} \, \frac{\partial^3 \uL}{\partial x^3} = \RHS
%\qquad \text{for } x \in [-\pi,\pi]
\end{align}	
where we have represented the right-hand side by $\RHS$.
...
\end{equation}
With this spectral discretization, the ODE system \eqref{TKdV2} can be written for the spectral vector $\mathbf{u}=\left\{ \hat{u}_{k}\right\}_{k=0}^{\Lambda}$ and $\mathbf{F}=\left\{ \RHSh_{k} \right\}_{k=0}^{\Lambda}$ as
\begin{equation}
\label{TKdVDisc}
...
For time integration of \eqref{TKdVDisc}, we employ a 4th-order midpoint symplectic scheme \cite{mclachlan1993symplectic}. Let $\mathbf{u}^{n}$ be the solution at time $t_{n}$ and $\mathbf{u}^{n+1}$
...
\begin{align}
&\mathbf{y}_{1}-\mathbf{u}^{n} = w_{1}\Delta t\mathbf{F}\left(\frac{1}{2}\left[\mathbf{y}_{1}+\mathbf{u}^{n}\right]\right),\\
...
Then the PDFs can be counted from the solution $u_{\Lambda}\left(t\right)$, and the statistical expectations can be calculated by averaging along the trajectory
\begin{equation}
\left\langle F\left(u_{\Lambda}\right)\right\rangle _{\mu}=\frac{1}{T}\int_{t_{0}}^{t_{0}+T}F\left(u_{\Lambda}\left(t\right)\right)dt,
\end{equation}
given ergodicity in the solution with $u_{\Lambda}\left(t_{0}\right)$ already reaching the stationary state.
