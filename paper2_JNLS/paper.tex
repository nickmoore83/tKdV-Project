...
Our numerical scheme first separates the Hamiltonian into linear and nonlinear components. Then a pseudo-spectral scheme is used for spatial discretization with de-aliasing applied to the nonlinear term $\Proj (\uL)^2$ according to the standard 2/3-rule. That is, we first pad $\uhat_k$ with zeros for $\Lambda < \abs{k} \le 3 \Lambda$ \nick{3 or 3/2???}, transform to physical space and square to obtain $\uL^2$ on a fine grid, then transform back to spectral space and truncate to obtain the Fourier coefficients of $\Proj (\uL)^2$
...
%In the numerical discretization, first we split the Hamiltonian into separated linear and nonlinear dynamics. 


Therefore the ODE system after spatial discretization can be written for the spectral vector $\mathbf{u}=\left\{ \hat{u}_{k}\right\}_{k=0}^{\Lambda}$ and $\mathbf{F}=\left\{ \RHSh_{k} \right\}_{k=0}^{\Lambda}$
\begin{equation}
\dot{\hat{u}}_{k} = \RHSh_k \left[u\right] = C_{2}\mathcal{D}^{1/2}ik^{3}\hat{u}_{k}-\frac{1}{2}C_{3}\mathcal{D}^{-3/2}ik\widehat{\left(u^{2}\right)}_{k}.
\end{equation}
