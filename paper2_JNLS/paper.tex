...
\end{equation}
	...
	Importantly, \eqref{HtwoExpect} shows that the expected value of $\Htwo$ grows like $\Lambda^2$, which appears to be problematic for obtaining independence as $\Lambda \to \infty$. However, $\Htwo$ enters the definition of $\Ham$ in produce with the coefficient $C_2$, which itself depends on $\lamfac$ as $C_2 \sim \lamfac^{-2}$. Hence, obtaining the desired asymptotic independence of $\Lambda$ simply requires that $\lamfac$ grow proportionally to $\Lambda$.
