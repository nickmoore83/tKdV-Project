...
\end{align}
Here $\theta = \thupdn$ is the inverse temperature, which will take a different value on either side of the ADC, and $Z_{\theta}$ a constant that depends on $\theta$. Each measure $\Gupdn$ induces a corresponding ensemble average, denoted $\mean{}_{\pm}$. 
...
\todo[inline]{Here: Explain intuitive justification for matching condition}

\nick{The downstream Hamiltonian is not conserved upstream and will vary dynamically in upstream wavefield. Once the wavefield crosses the ADC, whatever value $\Hdn$ happens to be is locked in for the rest of the time (is conserved thereafter). Hence, the downstream hamiltonian must match at the ADC on a trajectory by trajectory basis. Therefore, it must match statistically.}
