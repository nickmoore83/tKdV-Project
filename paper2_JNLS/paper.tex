...
\author{
M.~N.~J.~Moore\thanks{United States Naval Academy}, 
...
% ORIGINAL: New experimental measurements confirm this prediction in spectacular fashion.

% OTHER
% New experimental measurements provide convincing evidence for this prediction.
% convincing, conclusive, compelling.
...
In summary, the TKdV system possesses three important invariants: momentum, energy, and Hamiltonian. 

\new{We comment on some important differences between the KdV and TKdV systems. The KdV system possesses in infinite sequence of additional invariants, associated with the fact that it is a completely integrable system. However, the analogues of these quantities are {\em not} generally invariants of the truncated system; no invariants beyond the three mentioned above are known to exist for TKdV. Hence, while KdV is a completely integrable system whose long-time dynamics are governed by soliton interactions, this same behavior should not be expected for TKdV. In particular, the long-time behavior is not simply governed by soliton motion and instead is chaotic and observed to be weakly ergodic \cite{bajars2013weakly}. In this way, the theory developed herein differs from, and could likely by complementary to, recent advances in the field of integrable turbulence \cite{zakharov2009turbulence, randoux2014intermittency, costa2014soliton, randoux2016nonlinear}, in which integrability is exploited in order to uncover statistical characteristics of the system of interest. 
}
\new{
We remind the reader that KdV is asymptotically valid for large spatial scales and hence truncating the small scales (or alternatively modeling them through closure) does not alter the order of approximation made. In this sense, KdV and TKdV should be viewed as on equal footing as far as physical realism.
Where differences exist in long-time dynamics, it is unclear which model, if either, more closely resembles reality.}
