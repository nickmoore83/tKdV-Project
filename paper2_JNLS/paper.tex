...
In addition, we provide a more thorough treatment of the link between experimental and model parameters in order to facilitate a detailed comparison between the two. 
In particular, we flesh out details of non-dimensionalization, which were only briefly addressed in prior work for the sake of brevity.
\end{comment}
...
\nick{Edit this}
To compute the transfer function, we use a weighted random sampling strategy, with samples drawn from the uniform distribution ($d\mu$ or $\Gz$, which notation?) and the weight provided by the Gibbs form \eqref{Gibbs}.
\begin{equation}
...
\end{equation}
For computing the transfer function, this simple, weighted averaging approach is preferable to more sophisticated MCMC because it allows you to collect a large number of samples once, and then simply change the weights for different $\theta$. MCMC would require to completely restart the sampling for each individual $\theta$.
...
%Two approaches can be applied to predict the statistics in the downstream wave disturbance, which can be used to confirm each other in the numerical results. The equilibrium statistical mechanics is an easy way to confirm the matching condition and get the equilibrium distribution in far-field long-time limit.  On the other hand, the direct simulations of the dynamical model can offer estimates about the transient statistics before the far-field equilibrium state is reached. 
...
%\qquad \text{for } x \in [-\pi,\pi]
\end{align}	
where we have represented the right-hand side by the operator $\RHS[\uL]$.
Our numerical scheme first separates the Hamiltonian into linear and nonlinear components. Then a pseudo-spectral scheme is used for spatial discretization with de-aliasing applied to the nonlinear term $\Proj (\uL)^2$ according to the standard 2/3-rule. That is, we first pad $\uhat_k$ with zeros for $\Lambda < \abs{k} \le 3\Lambda/2$, transform to physical space and square to obtain $\uL^2$ on a fine grid, then transform back to spectral space and truncate to obtain the Fourier coefficients of $\Proj (\uL)^2$
For simplicity, we denote these Fourier coefficients $\usqdah_k$. That is,
\begin{equation}
\Proj (\uL)^2 = \sum_{\abs{k} \le \Lambda} \usqdah_k e^{i k x}
\end{equation}
With this spectral discretization, the ODE system \eqref{TKdV2} can be written as
\begin{equation}
\label{TKdVDisc}
\td{}{t} \uhat_k =  -\frac{1}{2} C_3 \drat^{-3/2} \, ik \usqdah_k + C_2 \drat^{1/2} \, ik^{3} \uhat_k = \RHSh_k
\end{equation}
Due to the nonlinearity, this system mixes scales during evolution. We note the third-order linear term may become stiff for large $\Lambda$.


For time integration of \eqref{TKdVDisc}, we employ a 4th-order midpoint symplectic scheme \cite{mclachlan1993symplectic}. We introduce the spectral vectors $\mathbf{u}=\left\{ \hat{u}_{k}\right\}_{k=0}^{\Lambda}$ and $\mathbf{F}=\left\{ \RHSh_{k} \right\}_{k=0}^{\Lambda}$, and let $\mathbf{u}^{n}$ denote the solution at time $t_{n}$. The midpoint method has two intermediate stages and the auxiliary vectors $\mathbf{y}_{1},\mathbf{y}_{2}$:
\begin{align}
\label{SympSteps}
&\mathbf{y}_{1}-\mathbf{u}^{n} = w_{1}\Delta t \, \mathbf{F}
\left[ \frac{1}{2} \left(\mathbf{y}_{1}+\mathbf{u}^{n}\right) \right] ,\\
&\mathbf{y}_{2}-\mathbf{y}_{1} = w_{2}\Delta t \, \mathbf{F}
\left[ \frac{1}{2} \left(\mathbf{y}_{2}+\mathbf{y}_{1}\right) \right] ,\\
&\mathbf{u}^{n+1}-\mathbf{y}_{2} = w_{3}\Delta t \, \mathbf{F}
\left[ \frac{1}{2} \left(\mathbf{u}^{n+1}+\mathbf{y}_{2}\right) \right] ,
\end{align}
with the time increments $w_{1}=\left(2+2^{1/3}+2^{-1/3}\right)/3$,
$w_{2}=1-2w_{1}$, and $w_{3}=w_{1}$. 

\nick{We should simply comment that we use MHMC to initialize the direct simulations.}


\vsp{5}

\todo[inline]{Di, I do not understand this paragraph. Is the $F$ meant to be the same $F$ as above, i.e.~the right-hand-side of the TKdV equation \eqref{TKdV2}? Or is $F$ some arbitrary function of the state variable that we want to compute the expectation of? 
$F$ should be some arbitrary function for the expectation.}
Then the PDFs can be counted from the solution $\uL(t)$, and the statistical expectation of any functional $f$ can be calculated by averaging along the trajectory
\begin{equation}
\left\langle f \left(u_{\Lambda}\right)\right\rangle _{\mu}=\frac{1}{T}\int_{t_{0}}^{t_{0}+T} f \left(u_{\Lambda}\left(t\right)\right)dt,
\end{equation}
given ergodicity in the solution with $\uL(t_0)$ already reaching the stationary state.

\todo[inline]{I am also a little confused about the paragraphs below. Eq.~\eqref{iterate} is not the same form of Eq.~(43)--(45) because in those equations it is not simply $\mathbf{z}$ inside $\mathbf{F}$, but instead an average of $\mathbf{z}$ and the previous value. Am I simply misinterpreting?}
\todo[inline]{Also, this is simply Picard iteration, correct? Perhaps we could simply say in words that we perform Picard iteration so that we don't have to worry about getting the equations correct in detail. What's your opinion?}
...
%The symplectic numerical integrators discretize the Hamiltonian and the Poisson structures separated and then form the resulting scheme for conserving the Hamiltonian.
