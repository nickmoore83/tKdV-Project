...
%$C_3 = 0.20$ and $C_2 = 0.024$.
	...
	Finally, Fig.~\ref{trajectories} sheds some light on the timescales required for wave evolution in the TKdV framework. It appears that the normalized time of $t=10$ is roughly the correct timescale to observe substantial wave dynamics. More precisely, both the left-going waves in Fig.~\ref{trajectories}(a) and the right-going waves in Fig.~\ref{trajectories}(b) require a dimensionless time of about $t=7$ to traverse the entire periodic domain. For the representative experiments, the characteristic timescale, $\timescale$, is 2.8~sec upstream and 5.7~sec downstream. Hence, a dimensionless time of $t=7$ corresponds to roughly 20~sec and 110~sec upstream and downstream respectively. Given the wave speeds ($c = $ 110 cm/s upstream and 54 cm/s downstream), these timescales can be converted to a distance traveled by the waves. In fact, a simple calculation using the definitions in Section \ref{theory}, gives the distance traveled as $\traveldist = \lamfac g/(2 \pi \freqp^2)$. Notice the depth dependence has serendipitously canceled. Hence, the distance traveled by the waves over the dimensionless time of $t=7$ is the same value, 310 cm, both upstream and downstream. This distance is roughly half the 6 m length of the wave tank.

	These estimates raise an important question: if a distance of roughly 3 meters is required for significant wave evolution under the KdV framework, how is it that the experiments exhibit substantial changes in wave statistics a short distance, roughly 15 cm, downstream of the ADC? The results shown in Fig.~\ref{skew-kurt} help resolve this question. This figure shows the evolution of skewness (left) and kurtosis (right) of surface displacement computed by the TKdV deterministic simulations. The bold black curve shows the ensemble mean, while the skewness and kurtosis of 100 individual trajectories are shown by the faint gray curves. Importantly, the skewness and kurtosis evolve over a much shorter timescale than the aforementioned $t=7$ required for waves to cross the entire domain. More precisely, skewness and kurtosis saturate to their asymptotic value after a dimensionless time of roughly $t = 1$, corresponding 
