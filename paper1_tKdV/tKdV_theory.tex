...
\todo[inline]{If you like, we could cite papers on `Optical rogue waves' and/or rogue waves in microwave systems here. (references added)}
...
\todo[inline]{I am not sure we need to include this last item. It is not really an experimental observation, simply a calculation that can be done using the dispersion relation and the values of the two depths. Could we simply take this last item out? Would you prefer to replace it with another observation? I could easily come up with one. Perhaps a comment on the decay rate of the power spectra. (I will confirm with Andy whether he would just delete this item or keep something else like the one above)}
...
dynamical model. 
\todo[inline]{This first sentence seems awkward to me.}
...
\todo[inline]{I modified the last sentence above. Feel free to revert to your original sentence if you do not like mine. (the change looks good)}
...
\todo[inline]{Is there a difference between $L_0$ and $L_d$? typo? (These two are actually different. $L_d$ is the `true' length scale defined in SI eqn. [10]. I guess here it's better to delete this notation to avoid confusion.)}
...
\todo[inline]{In this section title, did you mean `statistical mechanism' or `statistical mechanics'? (typos corrected)}
...
\]
\todo[inline]{I do not believe we have defined $\mu_{\infty}^{-}$ anywhere. (removed to avoid confusion)}
...
confirm each other.
\todo[inline]{Again, do we mean statistical `mechanism' or `mechanics'? (should be `mechanics')}
