...
For the acceptance-rejection algorithm presented below, it will be important to find the maximal value of the Hamiltonian subject to the constraints of fixed energy $E = E_0$ and zero momentum $M = 0$. For simplicity, consider the KdV equation
\begin{equation}
u_t + u u_x + \mu u_{xxx} = 0
\end{equation}
with corresponding Hamiltonian
\begin{equation}
H = \frac{1}{6} \intt u^3 dx - \frac{\mu}{2} \intt u_x^2 dx
\end{equation}
...
\end{equation}
which is a second-order nonlinear ODE for the optimizer $u = U(x)$. Note that this is identical to the condition for a traveling wave solution!!!
