...
\subsubsection{My own reformulation}
...
Note, in the experiments, $\delta = 0.33$ and $\eps$ lies in the range $2.4 \times 10^{-3}$ to $2.4 \times 10^{-2}$. 
...
The main difficulty I have with this formulation is that there is no reason to expect the spatial period of $u$ to have anything to do with the tank width $L_0$. More logically, the spatial period of $u$ should correspond to a characteristic wavelength in the problem, either the peak or maximum wavelength. The latter is more likely to be true in a literal sense, but I believe the peak wavelength provides a more reasonable scale for non-dimensionalization.
