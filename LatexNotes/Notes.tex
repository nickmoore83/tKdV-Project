...
t^* = \eps^{-1/2} h_0/c
\end{align}
where $h_0$ is a characteristic depth (say the upstream depth), $c = \sqrt{g h_0}$ the corresponding wave speed, and $\eps = a / h_0$ is the ratio of wave amplitude to depth. We want to alter Johnson's non-dimensionalization so that $x$ is scaled on some characteristic wavelength (and $t$ scaled accordingly). Importantly, we need to account for variable depth.
...
Let's make one more cosmetic change. Divide Eq.~\eqref{varKdV1} by $3 \delta^2$ and set $\beta = 3\delta^2/2$ to get
\begin{align}
\label{varKdV}
u_s + \eps \delta^{-2} D^{-7/4} \, u u_x
+ \mu D^{1/2} \, u_{xxx} = 0
\end{align}
where
\begin{equation}
...
\begin{align}
\label{Hamiltonian}
& H^- = \eps \delta^{-2} H_3 - \mu H_2 \\
& H^+ = \eps \delta^{-2} D_0^{-7/4} H_3 - \mu D_0^{1/2} H_2
\end{align}

Note: $\eps \delta^{-2} = a f_p^2 / g$ which is an interesting parameter. It suggests an equivalence between driving amplitude and driving frequency.
