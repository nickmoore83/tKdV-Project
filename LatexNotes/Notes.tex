...
\item We have to scale the spatial variable (called $x$ or $\xi$) on a characteristic wavelength, which will be a different value on either side of the abrupt depth change. We have to do this so that the assumption of spatial periodicity will be reasonably satisfied.
\end{enumerate}
To achieve 3, we can be guided by the characteristic wavelength from shallow water theory
\begin{equation}
...
We therefore introduce dimensionless variables, denoted by a tilde
\begin{align}
