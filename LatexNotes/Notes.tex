...
\end{equation}
All variables above are dimensionless, and by working backwards I figured out how they relate to the original, dimensional variables. Below, the dimensional quantities are indicated with an overbar
\begin{align}
...
\end{align}
Here $d$ is the variable depth (which is dimensional) $c = \sqrt{g d}$ is the local wave speed, and $D = d/h_0$ is the variable depth ratio.
...
\item We have to scale the spatial variable (called $x$ or $\xi$) on a characteristic wavelength, which will be a different value on either side of the abrupt depth change. We have to do this so that the assumption of spatial periodicity will be reasonably satisfies.
\end{enumerate}
To achieve 2, we can be guided by the characteristic wavelength from shallow water theory
\begin{equation}
\lambda_c = {c}/{f_c} = \sqrt{g d}/f_c
\end{equation}
where $f_c$ is a characteristic forcing frequency and $c = \sqrt{g d}$ is the local wavespeed (in shallow-water theory). It is tempting to simply use the peak forcing frequency $f_p$ for $f_c$, which would result in the characteristic wavelength $\lambda_c$ being the peak wavelength $\lambda_p$. Perhaps a better choice, though, is to let $\lambda_c$ be an integer multiple of $\lambda_p$ to allow scales larger than the peak wavelength (which are certainly present in the experiments). Therefore, let us choose
\begin{equation}
\lambda_c = M \sqrt{gd} / f_p
\end{equation}
where $M$ is an integer (on the order of 10 or so).
...
\begin{align}
& X^* = \eps^{-1/2} h_0 ,\quad T^* = \eps^{-1/2} \sqrt{h_0/g} , \quad \eta^* = \eps h_0, \\
& Z^* = h_0 , \quad U^* = \eps \sqrt{g h_0} , \quad W^* = \eps^{3/2} \sqrt{g h_0} , \quad P^* = \eps \rho g h_0
\end{align}
...
\subsubsection{Reformulating variable-depth KdV}

We now seek to formulate the variable-depth KdV in a similar fashion. We start with Johnson's Eq. (3.148), which is written in dimensionless variables
\begin{equation}
2 \sqrt{D} \eta_X + \frac{1}{2} \frac{D'(X)}{\sqrt{D}} \eta + \frac{3}{D} \eta \eta_\xi + \frac{1}{3} D \eta_{\xi \xi \xi} = 0
\end{equation}
The first task is to convert this to dimensional form. For this, we now denote dimensionless variables with a tilde and use the relationships
\begin{align}
\eta = \eps h_0 \tilde{\eta} , \quad x = \eps^{-1/2} h_0 \tilde{x} = \eps^{-3/2} h_0 \tilde{X}
\end{align}
The trickiest part relates to $\xi$.
