...
To first-correction in small amplitude, surface displacements are governed by the Korteweg–de Vries equation, which in dimensional form is given by
\begin{equation}
...
\depth = 
\begin{cases}
\it \dpm \quad \mbox{if } x<0 \\
\it \dpp \quad \mbox{if } x>0
\end{cases}
\end{align}
Most often, we will consider waves moving into shallower depth, so that $\dpp < \dpm$.

\subsection{Nondimensionalization}

We introduce the following characteristic scales
\begin{align}
\mbox{\em characteristic wave amplitude} \qquad
a = \etastd = \mean{\eta^2}^{1/2} \\
\mbox{\em characteristic wave length} \qquad
