...
\end{cases}
\end{align}
Most often, we will consider waves moving into shallower depth, so that $\dup > \ddn$. The incoming waves are randomized and generated with a peak forcing frequency of $\freqp$. 
...
\end{align}
We will consider the physical domain $\xi \in [0, \lamfac \lam]$, i.e.~an integer multiple of the characteristic wavelength. Then (after a shift) the dimensionless domain can be taken to be $\tilde{x} \in [-\pi,\pi]$.
...
&\drat(x) = {\depth(x)}/{\dup} \, .
\end{align}
Here, $\drat = \drat(x)$ is the local dimensionless depth.
...
Upstream wavelength			& $\lamup = \sqrt{g \dup}/f_p$	& length		& 56 cm \\
Amplitude-to-depth ratio		& $\epsup = a / \dup$			& dimensionless	& 0.0024--0.024 \\
Depth-to-wavelength ratio		& $\delup = \dup / \lamup$		& dimensionless		& 0.22 \\
%Amplitude parameter			& $\ampp = \eps \delta^{-2} = a \lambda_0^2 h_0^{-3} = ag h_0^{-2} f_p^{-2}$	& dimensionless		& 0.05--0.5\\
...
\hline
Local depth			& $\depth$				& length \\
Local wavespeed		& $c = \sqrt{g \depth}$		& speed \\
Local peak wavelength	& $\lam = \sqrt{gd}/f_p$	& length \\
%Characteristic wavelength	& $\lambda_c = \lamfac \lambda_p$	& length \\
Depth ratio			& $\drat = \depth/\dup$		& dimensionless
\end{tabular}
\end{center}
\end{table}
 %^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^%
...
The above makes the implicit assumption of continuous surface displacement crossing the ADC. This is a very physically motivated assumption, but there is a small amount of debate surrounding the assumption. An alternative is to appeal to Green's law and assume the quantity $\depth^{1/4} \eta$ to be continuous across the ADC, which necessarily implies a small discontinuity in the surface (which of course cannot be true literally). The dimensional KdV equation would then be expressed as
\begin{equation}
2 (\depth^{1/4} \eta)_t + \frac{3 c}{\depth^{5/4}} (\depth^{1/4} \eta) (\depth^{1/4} \eta)_{\xi} + \frac{c \depth^2}{3} (\depth^{1/4} \eta)_{\xi \xi \xi} = 0
\end{equation}
This is the approach taken in Johnson's book. This assumption results in a small modification of the coefficients in the dimensionless KdV. In particular, it gives
\begin{equation}
...
\end{equation}
Notice the only changes are the definition of the state variable and the small change in the power of $\drat$ in the second term (changes from $\drat^{-1}$ to $\drat^{-5/4}$).
We will not use this alternative formulation.


\subsection{Hamiltonian analysis}

The domain of consideration is $x \in [-\pi,\pi]$. We first define the spatial average of any quantity as
\begin{equation}
\mean{q} = \frac{1}{2\pi} \int_{-\pi}^{\pi} q \dx \, .
\end{equation}
Note that from the definition of the characteristic amplitude in \eqref{ampdefn}, we have $\mean{u^2} = 1$. Consequently, the energy of $u$ is fixed as
\begin{equation}
