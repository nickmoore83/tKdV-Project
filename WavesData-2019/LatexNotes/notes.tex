...
&\drat(x) = {\depth(x)}/{\dup} \, .
\end{align}
$\drat = \drat(x)$ is the local dimensionless depth.
...
The domain of consideration is $x \in [-\pi,\pi]$. We first introduce notation for the spatial mean of any quantity $q$
\begin{equation}
\mean{q} = \frac{1}{2 \pi} \int_{-\pi}^{\pi} q \dx == \intt q \dx
\end{equation}


Note, that as a consequence of the definition of $\amp$ in \eqref{ampdefn}, we have that
\begin{equation}
E[u] = \frac{1}{2} \frac{1}{2 \pi} \int_{-\pi}^{\pi} u^2 \dx = \frac{1}{2}
\end{equation}
