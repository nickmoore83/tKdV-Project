...
+ \frac{1}{6} \left( \frac{c \depth^2 T}{L^3} \right) u_{xxx} = 0
\end{equation}
...
We will essential use the original normalization in \eqref{dimlessKdV1}, except with the slight modification that $\eta = \sqrt{\pi} \etastd u$, so that the energy of $u$ is normalized to unity. This choice will be consistent with our PNAS paper and the J Stat Phys paper of Majda and Qi.
...
\begin{equation}
C_3 \dratdn^{-1} \meandn{H_3} = C_2 \dratdn \left( \meandn{H_2} - \meanup{H_2} \right)
\end{equation}
Simplifying gives
\begin{equation}
\label{H3H2}
\meandn{H_3} = \frac{C_2}{C_3} \dratdn^2 \,\, \Delta \mean{H_2}
\end{equation}
where $\Delta \mean{H_2} =  \meandn{H_2} - \meanup{H_2}$  indicates the difference between the upstream and downstream values.
To convert to dimensional, i.e.~experimental, values we note
\begin{align}
&\mean{u^3} =  \frac{\mean{\eta^3}} {\pi^{3/2} \mean{\eta^2}^{3/2}} = \pi^{-3/2} \skw(\eta) \\
&\mean{u_x^2} = \pi^{-1} \left(\frac{\lamupdn}{\etastd} \right)^2 \mean{\eta_x^2} 
= \pi^{-1} \left(\frac{\lamupdn}{\etastd} \right)^2 \var(\eta_x)
\end{align}
Therefore
\begin{align}
& \meandn{H_3} = \frac{\pi}{3} \mean{u^3} = 
\frac{1}{3} \pi^{-1/2} \skw_+(\eta) \\
& \meanupdn{H_2} = \pi \meanupdn{u_x^2} = 
\left(\frac{\lamupdn}{\etastd} \right)^2 \var(\eta_x)
\end{align}
The second equation implies
\begin{equation}
\Delta \mean{H_2} =  \meandn{H_2} - \meanup{H_2} = 
\etastd^{-2} \left( \lamdn^2 \var_+(\eta_x) - \lamup^2 \var_{-}(\eta_x) \right) =
\etastd^{-2} \Delta \left( \lam^2 \var(\eta_x) \right)
\end{equation}
Then \eqref{H3H2} gives the relationship
\begin{equation}
\frac{\skw(\eta)} {\Delta \left( \lam^2 \var(\eta_x) \right) } =
\frac{3 \pi^{1/2} C_2 \dratdn^2}{C_3 \etastd^2}  =
\frac{\pi^2}{3 \lamfac^2} \delup^2 \epsup^{-1} \etastd^{-2} \dratdn^2 =
\frac{\pi^2}{3 \lamfac^2} \lamup^{-2} \epsup^{-3} \dratdn^2
\end{equation}
...
TO DO: Simplify by manipulating $\lam$ on both sides. Then could simplify $\Delta$ term on left with $\dratdn$ factor.
