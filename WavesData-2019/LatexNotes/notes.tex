...
Therefore, lets stick with the original normalization \eqref{dimlessKdV1}, recast here in different form
\begin{align}
\label{dimlessKdV2}
&u_t + C_3 \drat^{-1} \, u u_x + C_2 \drat \, u_{xxx} = 0
\qquad \text{for } x \in [-\pi,\pi] \\
&C_3 = \frac{3}{2} \pi \epsup  \\
&C_2 = \frac{\pi^3 \delup^2}{6 \lamfac^2} 
\end{align}
If we need to draw a comparison to the PNAS paper, we set something like $\epsup = E^{1/2}L^{-3/2}$ and $\delup^2 = L^{-3}$, etc.
...
\begin{equation}
\En[u] \defeq \frac{1}{2} \int_{-\pi}^{\pi} u^2 \dx = \pi
\end{equation}
...
\begin{equation}
\En[u] = \pi = \frac{1}{2} \int u^2 \dx = \pi \sum_{k=-\Lambda}^{\Lambda} \abs{\uhat_k}^2 = 2 \pi \sum_{k=1}^{\Lambda} \abs{\uhat_k}^2
\end{equation}
% The above agrees with the first line on top of p. 4 in Majda and Qi J Stat Phys 2019, except that the total energy is one instead of pi.
...
&\mean{u_x^2} = \left(\frac{\lamupdn}{a} \right)^2 \mean{\eta_x^2} 
= \left(\frac{\lamupdn}{a} \right)^2 \var(\eta_x)
\end{align}
Therefore
\begin{equation}
\meandn{H_3} = \frac{\pi}{3} \skw_+(\eta)
\end{equation}
and more subtle
\begin{equation}
\Delta \mean{H_2} =  \meandn{H_2} - \meanup{H_2} = 
\frac{\pi}{\amp^2} \left( \lamdn \var_+(\eta_x) - \lamup \var_{-}(\eta_x) \right) =
\frac{\pi}{\amp^2} \Delta \left( \lam \var(\eta_x) \right)
\end{equation}
Then \eqref{H3H2} gives the relationship
\begin{equation}
\frac{\skw(\eta)} {\Delta \left( \lam \var(\eta_x) \right) } =
\frac{3 C_2 \dratdn^2}{C_3 \amp^2}  =
% OLD 3 \mu D^2 \left( \frac{h_0}{a} \right)^3 = 3 \eps^{-3} \mu D^2
\end{equation}
FINISH
