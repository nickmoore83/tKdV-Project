...
\etastd^{-2} \left( \lamdn^2 \vardn(\eta_x) - \lamup^2 \varup(\eta_x) \right) =
...
\end{equation}
Performing some additional manipulations, and noting that $(\lamdn/\lamup)^2 = \ddn/\dup = \dratdn$, gives
\begin{equation}
\frac{\skwdn(\eta)} {\dratdn \vardn(\eta_x) - \varup(\eta_x)} = 
\frac{\pi^2}{3 \lamfac^2} \epsup^{-3} \dratdn^2
\end{equation}
...
% The old calculation used mu = 4 pi^2 / (9 N^2), but with a factor of two difference in the definition of N. The resulting coefficient on the RHS was 3 mu, which agrees with the RHS above once the factor of two is taken into account.
